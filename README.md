...
**Expect some waiting.** Cold sandbox start is fast (~200 ms), but each run installs Playwright + Chromium + system deps (`libnspr4`, `libnss3`, …) fresh, which takes **~90–120 s**. A full healing demo with 2–3 attempts runs in **3–6 min** wall clock. For repeat use, build a custom E2B template with everything pre-baked to drop back to the 200 ms floor.
...
- **`error while loading shared libraries: libnspr4.so`** — E2B's default `base` template lacks Chromium's system libraries. `runner.ts` runs `playwright install --with-deps chromium` to install them via apt-get.
